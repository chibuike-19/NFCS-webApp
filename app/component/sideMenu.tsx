...
const SideMenu = ({sidebarMenuData}: sidebarItems) => {
    ...
    const {toggleMenu, isMobile, showMenu} = useAuth()
    console.log(isMobile)
    return (
      <div className={`h-[100vh] ${isMobile && !showMenu ? 'hidden' : 'block'} ${showMenu ? 'absolute' : 'relative'} w-[280px] z-50 bg-white border-2 border-white transition-all duration-300 ease-in-out `}>
        <div className="flex justify-between px-4 py-8 items-center">
          ...
          {isMobile && <span onClick={toggleMenu}>
            <GrClose size={40}/>
          </span>}
        </div>
